...
if (isset($_POST['submit'])) {
    ...
    if (empty($errors)) {
        ...
        if ($avatar['name']) {
            ...
            move_uploaded_file($avatar['tmp_name'], $upload_dir . $avatar_name);
        }
        ...
        if ($stmt->affected_rows > 0) {
            ...
            unset($_SESSION['signup-data']); // Clear the form data session
            header('Location: ../signin-page.php'); // Redirect to a success page
            exit();
        } else {
    ...
    if (!empty($errors)) {
        $_SESSION['signup-error'] = implode('<br>', $errors);
        header('Location: ../signup-page.php'); // Redirect back to the signup page
        exit();
    }
}
?>
